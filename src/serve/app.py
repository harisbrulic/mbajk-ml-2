...
RAW_DATA_DIR = os.path.join(ROOT_DIR, 'data', 'raw', 'weather')
...
def predict(station_name):
    ...
    try:
        ...
        if data_array.shape != (5, num_features):
            raise ValueError("Incorrect data shape. Expected 5 timesteps with 5 features each.")

        data_array = data_array.reshape(-1, num_features)
        data_normalized = scaler.transform(data_array)

        data_reshaped = data_normalized.reshape(1, 5, num_features) 
...
def load_scaler(station_name):
    ...
    scaler_file = os.path.join(scaler_path, station_name.replace(" ", "-") + "_processed_data_scaler.pkl")
